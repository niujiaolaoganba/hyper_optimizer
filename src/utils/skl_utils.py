...
@brief: 定义skl模型工具，包括SVR，LinearSVR, KNNRegressor, AdaBoostRegressor, RandomRidge
@todo：
dist_utils
"""
...
class AdaBoostRegressor:
    def __init__(self, base_estimator=None, n_estimators=50,max_features=1.0,
        ...
        if base_estimator and base_estimator == "etr":
            base_estimator = ExtraTreeRegressor(max_depth=max_depth, max_features=max_features)
        else:
            base_estimator = DecisionTreeRegressor(max_depth=max_depth, max_features=max_features)

        self.model = sklearn.ensemble.AdaBoostRegressor(base_estimator=base_estimator,
...
class RandomRidge:
    def __init__(self, alpha=1.0, normalize=True, poly=False, n_estimators=10,
                 max_features=1.0, bootstrap=True, subsample=1.0, random_state=2016):
        self.alpha = alpha
    ...
    def predict(self, X):
        ...
        for i in range(self.n_estimators):
            ...
            X_tmp = X[:, fidx]
            if self.poly:
                X_tmp = PolynomialFeatures(degree=2).fit_transfrom(X_tmp)[:, 1:]
